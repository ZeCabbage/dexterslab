...
class TTSReceiver:
    ...
    def restart(self):
        """Restart the WS connection thread."""
        logger.info("[TTSReceiver] Restarting connection thread...")
        if self._thread and self._thread.is_alive():
            self._thread.join(timeout=2)
        self._thread = threading.Thread(target=self._run_loop, daemon=True)
        self._thread.start()
    ...
    def _run_loop(self):
        ...
        try:
            ...
            logger.error(f"[TTSReceiver] WS loop crashed: {e}")
        finally:
            loop.close()
        ...
        while self._running:
            try:
                ...
                if not self._running:
                    break
                logger.warning(f"[TTSReceiver] Connection failed ({type(e).__name__}: {e}). Reconnecting in {backoff}s")
    ...
    def _speak(self, text: str):
        ...
        try:
            # Ensure USB speaker is at 90% volume before every speak
            # Uses config-driven card number to handle card renumbering across reboots
            card = str(self.config.audio_playback_card)
            subprocess.run(
