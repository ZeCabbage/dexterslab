...
Reconnection: Uses non-blocking async queue reads and an active watchdog
to detect stale connections (e.g. after PC sleep/wake) and force reconnect.
"""
...
class VideoStreamer:
    ...
    async def _async_ws_loop(self):
        """
        Robust async WebSocket loop with:
        - Non-blocking queue reads (run_in_executor) to avoid blocking the event loop
        - Active ping watchdog to detect stale connections
        - Forced reconnect after 30s of no successful sends
        """
        ...
        while self._running:
            try:
                async with websockets.connect(
                    ...
                    while self._running:
                        try:
                            # Non-blocking queue read via executor
                            loop = asyncio.get_event_loop()
                            try:
                                frame = await asyncio.wait_for(
                                    loop.run_in_executor(
                                        None,
                                        lambda: self._frame_queue.get(timeout=0.3)
                                    ),
                                    timeout=1.0
                                )
                            except (asyncio.TimeoutError, queue_module.Empty):
                                # No frames available — check connection health
                                if time.time() - last_send_ok > 30:
                                    logger.warning("[VideoStreamer] No successful send in 30s, forcing reconnect")
                                    break
                                ...
                                try:
                                    ...
                                    logger.warning("[VideoStreamer] Ping failed, connection dead")
                                    break
                                continue

                            # Send the frame with a timeout
                            await asyncio.wait_for(ws.send(frame), timeout=5)
                            last_send_ok = time.time()

                        except (websockets.exceptions.ConnectionClosed,
                                websockets.exceptions.ConnectionClosedError,
                                websockets.exceptions.ConnectionClosedOK) as e:
                            logger.warning(f"[VideoStreamer] Connection closed: {e}")
                            break
                        except asyncio.TimeoutError:
                            logger.warning("[VideoStreamer] Send timed out, reconnecting")
                            break
                        except Exception as e:
                            logger.warning(f"[VideoStreamer] Send error: {e}")
                            break
