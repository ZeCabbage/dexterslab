...
export default function LevelUpModal({ onClose }: Props) {
  ...
  // State: Feature Choices (Invocations, Fighting Styles, etc.)
  const needsInvocations = targetClass === 'Warlock' && targetClassLevel >= 2;
  const numInvocationsDesired = targetClass === 'Warlock' ? (targetClassLevel >= 18 ? 8 : targetClassLevel >= 15 ? 7 : targetClassLevel >= 12 ? 6 : targetClassLevel >= 9 ? 5 : targetClassLevel >= 7 ? 4 : targetClassLevel >= 5 ? 3 : targetClassLevel >= 2 ? 2 : 0) : 0;
  // Fallback to checking how many we ALREADY have on the character!
  // Count existing:
  const existingInvocations = (char?.features || []).filter((f: FeatureData) => Object.keys(ELDRITCH_INVOCATIONS).includes(f.id || ''));
  const invocationsToDraft = numInvocationsDesired - existingInvocations.length;
  const needsChoices = invocationsToDraft > 0;
  const [draftedChoices, setDraftedChoices] = useState<FeatureData[]>([]);
  ...
  if (needsChoices) stepsList.push('Class Choices');
  ...
  const invokeOracleSubclassFeatures = async () => {
    const finalSubclass = subclassChoice || char?.subclasses?.[targetClass];
    if (!finalSubclass) return;
    
    setIsOracleForging(true);
    try {
      const res = await fetch('/api/dungeon-buddy/oracle/forge-subclass-features', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ subclassName: finalSubclass, charClass: targetClass, nextLevel: targetClassLevel })
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      ...
      if (data.features && data.features.length > 0) {
        ...
        alert(`The Oracle confirms: Level ${targetClassLevel} grants no specific new subclass features for the ${finalSubclass}.`);
      }
    } catch (err: any) {
      alert("Oracle Extraction Failed: " + err.message);
    } finally {
      setIsOracleForging(false);
    }
  };
  ...
  const renderStep = () => {
    ...
    if (currentView === 'Grimoire (Spells)') {
     ...
     if (currentView === 'Class Choices') {
       return (
         <div style={{ animation: 'fadeIn 0.3s' }}>
           <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
             <div>
               <h3 style={{ color: '#cfaa5e', fontFamily: 'Cinzel', marginBottom: '8px' }}>Eldritch Invocations</h3>
               <p style={{ color: '#aaa', fontSize: '14px', margin: 0 }}>You have unlocked forbidden knowledge. Select {invocationsToDraft} invocations.</p>
             </div>
             <div style={{ background: '#111', border: `1px solid ${draftedChoices.length >= invocationsToDraft ? '#cfaa5e' : '#444'}`, padding: '8px 16px', borderRadius: '4px', textAlign: 'center' }}>
                <div style={{ fontSize: '20px', fontFamily: 'Cinzel', color: draftedChoices.length > invocationsToDraft ? '#a44' : '#cfaa5e' }}>
                   {draftedChoices.length} / {invocationsToDraft}
                </div>
             </div>
           </div>

           <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
             {Object.values(ELDRITCH_INVOCATIONS).map(inv => {
               const isAlreadyOwned = existingInvocations.some((f: FeatureData) => f.id === inv.id);
               const isDrafted = draftedChoices.some(f => f.id === inv.id);
               
               if (isAlreadyOwned) return null; // Hide already owned
               
               return (
                 <div key={inv.id} style={{
                   background: isDrafted ? 'rgba(207, 170, 94, 0.1)' : '#111',
                   border: `1px solid ${isDrafted ? '#cfaa5e' : '#333'}`,
                   padding: '12px', borderRadius: '6px',
                   display: 'flex', justifyContent: 'space-between', alignItems: 'center'
                 }}>
                   <div>
                     <h4 style={{ margin: '0 0 4px', color: isDrafted ? '#fff' : '#cfaa5e', fontSize: '14px' }}>{inv.name}</h4>
                     <p style={{ margin: 0, fontSize: '12px', color: '#888' }}>{inv.description}</p>
                   </div>
                   <button 
                     onClick={() => {
                        if (isDrafted) {
                           setDraftedChoices(draftedChoices.filter(f => f.id !== inv.id));
                        } else {
                           if (draftedChoices.length >= invocationsToDraft) {
                              alert(`You can only select ${invocationsToDraft} choice(s) right now.`);
                              return;
                           }
                           setDraftedChoices([...draftedChoices, inv]);
                        }
                     }}
                     style={{
                        padding: '6px 12px',
                        background: isDrafted ? '#cfaa5e' : 'transparent',
                        color: isDrafted ? '#000' : '#cfaa5e',
                        border: `1px solid ${isDrafted ? '#cfaa5e' : '#555'}`,
                        borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold'
                     }}
                   >
                     {isDrafted ? 'DRAFTED' : 'SELECT'}
                   </button>
                 </div>
               );
             })}
    ...
    if (currentView === 'Class Features') {
      return (
        <div style={{ animation: 'fadeIn 0.3s' }}>
          ...
          <div style={{ borderTop: '1px dashed #333', paddingTop: '16px' }}>
             <h4 style={{ color: '#55aacc', fontFamily: 'Cinzel', marginBottom: '8px' }}>The Subclass Oracle</h4>
             <p style={{ fontSize: '12px', color: '#aaa', marginBottom: '12px' }}>
               If your chosen path (<strong>{subclassChoice || char?.subclasses?.[targetClass] || 'None'}</strong>) grants mechanics at this level, ask the Oracle to manifest them perfectly according to standard 5E lore.
             </p>
             <button 
               onClick={invokeOracleSubclassFeatures}
               disabled={isOracleForging || !(subclassChoice || char?.subclasses?.[targetClass])}
               style={{ width: '100%', padding: '12px', background: '#0f2537', color: '#55aacc', border: '1px solid #55aacc', borderRadius: '4px', cursor: (!(subclassChoice || char?.subclasses?.[targetClass]) || isOracleForging) ? 'not-allowed' : 'pointer', fontWeight: 'bold', textShadow: '0 0 5px rgba(85,170,204,0.5)', transition: 'all 0.2s', opacity: (!(subclassChoice || char?.subclasses?.[targetClass]) || isOracleForging) ? 0.5 : 1, marginBottom: '24px' }}
             >
               {isOracleForging ? 'Divining the Lore...' : `+ Channel ${subclassChoice || char?.subclasses?.[targetClass] || 'Subclass'} Features (Level ${targetClassLevel})`}
             </button>
          </div>
          ...
          <div style={{ borderTop: '1px dashed #333', paddingTop: '16px' }}>
            ...
            {customFeatures.map((feat, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '12px', background: '#111', padding: '12px', borderRadius: '4px', border: '1px solid #cfaa5e' }}>
                <div style={{ display: 'flex', gap: '8px' }}>
                   <input placeholder="Feature Name" value={feat.name} onChange={e => { const copy=[...customFeatures]; copy[i].name=e.target.value; setCustomFeatures(copy); }} style={{ flex: 1, padding: '8px', background: '#000', border: '1px solid #444', color: '#fff', fontWeight: 'bold' }} />
            ...
            <button onClick={() => setCustomFeatures([...customFeatures, {name:'', description:''}])} style={{ width: '100%', padding: '8px 16px', background: 'transparent', border: '1px solid #cfaa5e', color: '#cfaa5e', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', textAlign: 'center' }}>
              + Add Manual Feature
            </button>
          </div>
        </div>
